...
const BUDGETS = [
  ...
  { name: 'qo\'llanmalar (guides/inline/)', dir: 'guides/inline', ext: ['.html'],
    maxTotalKb: 700, maxFileKb: 110, gzipMaxFileKb: 30 }
];
...
// o'smasin). Chegara 420 -> 450 -> 470 -> 480 -> 520 -> 560 KB: birinchisi
// bo'limlarga qo'shilgan ohang va katta kartochkalar uchun, ikkinchisi rus
// tili lug'ati to'ldirilgani uchun (butun qo'llanmalar ekrani tarjimasiz
// qolgan edi), uchinchisi brend introsining ~7 KB lik dvigateli uchun,
// to'rtinchisi 43 do'konning xususiyat qatorlari va pullik xizmatlar
// ekranining tarjimasi uchun, beshinchisi chuqur ekranlarning ruscha
// lug'ati uchun (qidiruv, kuzatuv, papkalar, bojxonaning oltita bo'limi —
// 200 ga yaqin satr tarjimasiz turgan edi; gzip 115 -> 126 KB).
...
if (existsSync(index)) {
  ...
  /* 840 KB: TZ bosqichlari uchun 680 → 720 → 760, skrinshot natija ekrani
     uchun 800, "Nima mahsulot qidiryapsiz?" savol ekrani, ovozli kiritish
     va Markaziy bank valyuta jadvali (2026-09-18) uchun 840 KB ga
     oshirildi — README, "O'lcham byudjeti". Foydalanuvchi yuklaydigan
     haqiqiy hajm — gzip ≈ 192 KB. */
  const ok = size <= 840;
  console.log(`${ok ? '  ok  ' : ' XATO '} index.html: ${size} KB (chegara 840 KB), gzip ${gz} KB`);
  if (!ok) failed = true;
}
